...
my_files <- list.files(path=("card_data/"), pattern="*.csv", full.names = TRUE)

# Read in files with the tidyverse, removing rows with all NA, and correcting a typo
cards <- read_csv(my_files, guess_max = 300)
# 
...
# Finding Cards for the Card Showcase
...
```{r}
read_csv("app/cool_cards.csv") %>% 
  select(-id) %>% 
  mutate(image_front = paste(tolower(substr(name, 1, 4)) %>% str_remove_all(" "),
                             year,
                             substr(brand, 1, 3) %>% str_remove_all(" "),
                             str_remove(card_number, "-") %>% tolower(),
                             "f.png",
                             sep = "_"),
         image_rear = paste(tolower(substr(name, 1, 4)) %>% str_remove_all(" "),
                             year,
                             substr(brand, 1, 3) %>% str_remove_all(" "),
                             str_remove(card_number, "-") %>% tolower(),
                             "r.png",
                             sep = "_")) %>% 
  write_csv("app/cool_cards.csv")

cools <- read_csv("app/cool_cards.csv")
```


```{r}
test <- read_csv("card_data/arizona_diamondbacks.csv") %>% 
  mutate(numbered = as.character(numbered)) %>% 
  left_join(cools) %>% 
  select(name:imitation, showcase, notes, image_front, image_rear)
```
